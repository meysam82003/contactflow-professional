...
const pageMeta = {
  ...
  cloud:['حساب و Sync','Backup رمزگذاری‌شده و همگام‌سازی چند دستگاه'],
  telegram:['تلگرام','Mini App و Business Private Chat Workspace'],
...
function setBusy(b){state.busy=b; $('start-import').disabled=b; $('start-rename').disabled=b; $('start-export').disabled=b;}
...
function makeExportBlob(format,rows){if(format==='vcf')return new Blob([rows.map(vcfLine).join('')],{type:'text/vcard;charset=utf-8'});return new Blob(['\ufeffname,phone,city,section,source\r\n'+rows.map(v=>[v.name,v.phone,v.city,v.section||'',v.source].map(csvCell).join(',')).join('\r\n')],{type:'text/csv;charset=utf-8'});}
