...
- Backup دستی اکنون Blob/تصویر را واقعاً نگه می‌دارد، SHA‑256 و شمارش هر مخزن را پیش از Restore بررسی می‌کند و کل دیتابیس را در یک Transaction بازیابی می‌کند؛ APK فایل بزرگ را قطعه‌ای در مقصد انتخابی می‌نویسد.
- ورود VCF افزونهٔ مخاطبین پیش از شروع فایل را می‌خواند و دو مقصد «ورود سریع» و «ورود به گوشی» را با دکمه‌های صریح نشان می‌دهد؛ مشکل پنجره‌ای که فقط «لغو» داشت برطرف شده است.
