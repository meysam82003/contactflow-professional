...
test('all canonical version markers are 3.4.0',()=>{
  assert.equal(read('VERSION').trim(),'3.4.0');
  assert.equal(JSON.parse(read('package.json')).version,'3.4.0');
  assert.match(read('web/app.js'),/APP_VERSION = '3\.4\.0'/);
  assert.match(read('web/runtime-patch.js'),/VERSION='3\.4\.0'/);
  assert.match(read('web/sw.js'),/v3\.4\.0/);
  assert.match(read('desktop/main.go'),/appVersion = "3\.4\.0"/);
  assert.match(read('android/app/build.gradle'),/versionName '3\.4\.0'/);
  assert.match(read('telegram-miniapp/lib.php'),/CF_VERSION = '3\.4\.0'/);
});

test('shared web core includes smart merge and Telegram contact modules',()=>{
  const html=read('web/index.html');
  for(const asset of ['contact-export.js','telegram-web.bundle.js','runtime-patch.js','import-merge.js','v34.js','v34.css'])assert.ok(html.includes(asset),`index missing ${asset}`);
...
test('release workflow builds all supported device artifacts',()=>{
  ...
  assert.match(workflow,/tag_name: v3\.4\.0/);
});
