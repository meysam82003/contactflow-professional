...
test('all canonical version markers are 3.5.0',()=>{
  assert.equal(read('VERSION').trim(),'3.5.0');
  assert.equal(JSON.parse(read('package.json')).version,'3.5.0');
  assert.match(read('web/app.js'),/APP_VERSION = '3\.5\.0'/);
  assert.match(read('web/runtime-patch.js'),/VERSION='3\.5\.0'/);
  assert.match(read('web/sw.js'),/v3\.5\.0/);
  assert.match(read('desktop/main.go'),/appVersion = "3\.5\.0"/);
  assert.match(read('android/app/build.gradle'),/versionName '3\.5\.0'/);
  assert.match(read('telegram-miniapp/lib.php'),/CF_VERSION = '3\.5\.0'/);
});

test('shared web core includes smart merge and Telegram contact modules',()=>{
  const html=read('web/index.html');
  for(const asset of ['contact-export.js','telegram-web.bundle.js','runtime-patch.js','import-merge.js','location-operator.js','channel-handoff.js','v34.js','v34.css','v35.js','v35.css'])assert.ok(html.includes(asset),`index missing ${asset}`);
...
test('3.5 preserves more than fifty 3.4 shared feature contracts',()=>{
...
test('release workflow builds all supported device artifacts',()=>{
  ...
  for(const marker of ['_PWA.zip','_Windows_Portable.exe','_Windows_Setup.exe','_Linux_x64','_macOS_Intel','_macOS_AppleSilicon','_Android.apk','_Telegram_MiniApp_cPanel.zip','_Source.zip'])assert.ok(workflow.includes(marker),`workflow missing ${marker}`);
  assert.match(workflow,/tag_name: v3\.5\.0/);
});
