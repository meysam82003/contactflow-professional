...
function updateVersion(){
  document.title='ContactFlow Personal Ultimate 3.3';document.body.dataset.runtime=runtimeKind();
  document.querySelectorAll('.brand span,.setting-box code,.callout strong').forEach(el=>{el.textContent=el.textContent.replace(/3\.[01](?:\.0(?:-alpha\.1)?)?/g,'3.3.0')});
  let meta=document.querySelector('meta[name="application-name"]');if(!meta){meta=document.createElement('meta');meta.name='application-name';document.head.appendChild(meta)}meta.content='ContactFlow 3.3';
}
